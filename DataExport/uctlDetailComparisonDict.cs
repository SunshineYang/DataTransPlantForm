...
namespace DataExport
{
    ...
    public partial class uctlDetailComparisonDict : UserControl
    {
        ...
        {
            cmb_pt_name.DataSource = DALUse.Query(string.Format(clsPublicProperty.sqlGetPT)).Tables[0];
        ...
        private void btn_dellocaldict_Click(object sender, EventArgs e)
        {
            try
            {
                string sql = string.Format("delete PT_LOCAL_DICT where pt_id = '{0}' and type_name = '{1}'", cmb_pt_name.SelectedValue.ToString(), cmb_local.SelectedValue.ToString());
                if (MessageBox.Show(null, "确定删除所选字典？", "", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    if (DALUse.ExecuteSql(sql) > 0)
                    {
